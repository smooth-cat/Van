...
export class PriorityQueue<T> {
  ...
  log: Function = () => {};
  hasLog = false;
  constructor(compare: (a: T,b: T) => number, log?: Function) {
    if(log) {
      this.log = log;
      this.hasLog = true;
    }
    this.aIsUrgent = (a,b) => compare(a,b) < 0;
    this.arr = [];
  }
  ...
  goUp = (arr: T[], current: T, len: number) => {
    ...
    while (i > 0) {
      ...
      if(this.aIsUrgent(item, parent)) {
        this.log(`交换 parent:${parent} -> child:${item} `);
        exchange(arr, i, pI);
        this.logTree();
        i = pI;
      } else {
        this.log(`parent:${parent} child:${item} 不需要交换 \n`);
        break;
      }
    }
  }
  ...
  poll() {
    const { arr } = this;
    this.log(`弹出 ${arr[0]} 把 ${arr[arr.length - 1]} 放置到队头 `);
    const len = this.size();
    if(len <= 2) {
      return arr.shift()
    } 
  ...
  goDown = (arr: T[], i: number) => {
    ...
    while (i < half) {
      ...
      this.log(`交换 parent:${arr[i]} -> child:${arr[point]} `);
  ...
  logTree() {
    ...
    matrix.forEach((it) => {
      ...
      this.log(str.padStart(halfLen+halfIt, ' '))
    })
    this.log('\n');
