...
import { eqPos, getShotPrefixText, isFormer, lastFit, searchUpperSymbol } from '../../shared/utils';
...
export const DetailFile: FC<Data, Props> = (data, props) => {
  ...
  function gotoUpper(upperSymbol: DocNode | undefined, destroy: () => void) {
    ...
    msg.request(ReqType.Command, [
      'gotoLocation',
      toRaw(props.uri),
      toRaw(start),
      { triggerEvent: true, forceRefresh: true }
    ]);
    destroy();
  }
    ...
    return [
      data.loadingSymbol
        ...
        : el(
            'div',
            {
              class: cNames('popup-upper', { 'can-click': !!upperSymbol }),
              onclick: () => gotoUpper(upperSymbol, destroy)
            },
            [
              el('div', { class: 'label', style: `${labelStyle}` }, [text(upper === 'global' ? 'global' : type)]),
              el('div', { class: 'name', style: conf.TextUseTagColor ? nameStyle : '' }, [text(upper)])
            ]
          )
    ];
  }
    ...
    return [
      el('div', { class: 'ref-file', onmouseenter: getRefFileSymbol }, [
        el(
          'div',
          {
            class: cNames('file-title', { 'active-file': isHistoryItem ? cursor.i === index : uri.active }),
            onclick: clearFile
          },
          [
            // TODO: 使用 icon-font
            ...
            el(
              ...
              showedRefs.reduce((lis, it: Reference, j) => {
                const isDefine = !define
                  ? false
                  : define.range[0].line === it.range[0].line && define.range[0].character === it.range[0].character;
                const isActive = isHistoryItem ? cursor.i === index && cursor.j === j : it.active;
                const shotPrefix = isHistoryItem ? getShotPrefixText(it.prefix) : it.prefix;
                ...
                lis.push(
                  ...[
                    ...
                    el(
                      ...
                      [
                        el('div', { class: 'ref-lineText fade-ellipsis', title: it.prefix + it.name + it.suffix }, [
                          text(shotPrefix),
